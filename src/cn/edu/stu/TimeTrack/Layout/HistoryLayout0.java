...
public class HistoryLayout0 extends LinearLayout {//implements CalendarView.OnCellTouchListener {
...
//	private CalendarView calendarView;
	...
	int[] colors = new int[] { R.color.relax, R.color.entertm, R.color.study, R.color.work };
	int[] shade_colors = new int[] {Color.rgb(20, 181, 181),Color.rgb(134,105,183),Color.rgb(221, 195, 86),Color.rgb(15,150,197)};

	public HistoryLayout0(MainActivity context) {
		super(context);
		// TODO Auto-generated constructor stub
		mContext = context;
		// intent = mContext.getIntent(); // ！！！注意引用的是主Activity的，
		Calendar calendar = Calendar.getInstance();
		mdate = calendar.get(Calendar.YEAR) + "-" + (calendar.get(Calendar.MONTH) + 1) + "-"
	...
	private void init() {
		...
		llHistoryDay = LinearLayout.inflate(mContext, R.layout.history_day, null);
...
//		tvDayDate.setText(calendarView.getYear() + "-" + String.format("%1$02d", calendarView.getMonth() + 1));
		...
		llHistoryMonth = LinearLayout.inflate(mContext, R.layout.history_month, null);
		imMonthPieChart = (ImageButton) llHistoryMonth.findViewById(R.id.im_month_piechart);
		imMonthLineChart = (ImageButton) llHistoryMonth.findViewById(R.id.im_month_linechart);
		llChangeMonth = (LinearLayout) llHistoryMonth.findViewById(R.id.ll_change_month);
		// llMonthContent = (LinearLayout) llHistoryMonth
		// .findViewById(R.id.ll_month_content);
		llMonthLine = View.inflate(mContext, R.layout.history_month_linechart, null);
		// lineChartView2 =
		// (LineChartView)llMonthLine.findViewById(R.id.linechart);
		llScrollMonth = (LinearLayout) llMonthLine.findViewById(R.id.ll_scroll_month);

		llMonthItem = View.inflate(mContext, R.layout.history_month_item, null);
		llMonthContent = (LinearLayout) llMonthItem.findViewById(R.id.ll_month_content);
...
//		imMonthLeft.setOnClickListener(new CalendarMonthBntOnClickListener());
//		imMonthRight.setOnClickListener(new CalendarMonthBntOnClickListener());
...
//				tvHistoryWeek.setBackgroundResource(R.drawable.day_w);
//				tvHistoryMonth.setBackgroundResource(R.drawable.day_m);
				llHistoryContent.removeAllViews();
				llHistoryContent.addView(llHistoryDay);
				break;
			case R.id.tv_history_week:
//				tvHistoryDay.setBackgroundResource(R.drawable.week_d);
//				tvHistoryWeek.setBackgroundResource(R.drawable.week);
//				tvHistoryMonth.setBackgroundResource(R.drawable.week_m);
				llHistoryContent.removeAllViews();
				llHistoryContent.addView(llHistoryWeek);
				imWeekPieChart.performClick();
				break;
			case R.id.tv_history_month:
				tvHistoryDay.setBackgroundResource(R.drawable.month_d);
				tvHistoryWeek.setBackgroundResource(R.drawable.month_w);
...
//			// 在此让当前的View 重绘一次
//			clickdate = (calendarView.getYear() + "-" + (1 + calendarView.getMonth()) + "-" + cell.getDayOfMonth());
...
//					|| (calendar.get(Calendar.DAY_OF_MONTH) < clickday && calendar.get(Calendar.MONTH) + 1 == clickmonth)) {
//			} else {
...
//	}
	...
	private class WeekOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			if (imWeekPieChart == v) {
				imWeekPieChart.setBackgroundResource(R.drawable.today_pie_chart2);
				imWeekLineChart.setBackgroundResource(R.drawable.today_line_chart);
				isPieClicked = true;
				...
				imWeekPieChart.setBackgroundResource(R.drawable.today_pie_chart);
				imWeekLineChart.setBackgroundResource(R.drawable.today_line_chart2);
				isPieClicked = false;
	...
	private class MonthOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (imMonthPieChart == v) {
				imMonthPieChart.setBackgroundResource(R.drawable.today_pie_chart2);
				imMonthLineChart.setBackgroundResource(R.drawable.today_line_chart);
				...
				imMonthPieChart.setBackgroundResource(R.drawable.today_pie_chart);
				imMonthLineChart.setBackgroundResource(R.drawable.today_line_chart2);
				chgeWM.isWeek = false;
	...
	private class ChangeWeekOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (ibWLast == v) {
				...
				// 判断当前的开始日期是否与四周前的相同
				if (chgeWM.strWdate.get(chgeWM.strWdate.size() - 1).equals(chgeWM.preFourWeeks(ldate))) {
				} else {
					date = chgeWM.datePreDay(chgeWM.strWdate.get(chgeWM.strWdate.size() - 1));
